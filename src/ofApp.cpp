...
void ofApp::setup(){
    ...
    gui.setup();
    //0.818135 0.8652208
    gui.add(strutFactor.setup("strutFactor", 0.2, -0.02, 4.0)); //支柱の長さ
    gui.add(circleResolution.setup("circle res", 5, 5, 9)); //頂点数
    gui.add(maxlevels.setup("max level", 2, 2, 7)); //繰り返し数
    gui.add(lineWidth.setup("lineWidth", 6, 5, 10)); //ラインの太さ
    gui.add(startAngle.setup("angle", -90, -360, 360)); //角度
...
void ofApp::keyReleased(int key){
    ...
    if (key == 'S' || key == 's') {
        myImage2.save(ofGetTimestampString("%Y%m%d%H%M%S")+"##.jpg");
    }
    ...
    if (key == '6') {
        colors.clear();
        colors.push_back(ofColor(204, 229, 153)); //0 //yellow green
        colors.push_back(ofColor(colors[0].r, colors[0].g, colors[0].b, ofRandom(150, 240))); // 1
        colors.push_back(ofColor(243, 243, 243)); //2 //white
        colors.push_back(ofColor(243, 243, 243)); //3
        colors.push_back(ofColor(233, 56, 104)); //4 //red
        colors.push_back(ofColor(colors[4].r, colors[4].g, colors[4].b, ofRandom(150, 240))); //5
        colors.push_back(ofColor(204, 231, 227)); //6 //light blue
        colors.push_back(ofColor(colors[6].r, colors[6].g, colors[6].b, ofRandom(150, 240))); //7
        colors.push_back(ofColor(253, 128, 26)); //8 //orange
        colors.push_back(ofColor(colors[10].r, colors[10].g, colors[10].b, ofRandom(150, 240))); //9
